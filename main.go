...
func main() {
	...
	if appendToOut {
		...
		if len(imports) > 0 && (typed || genericTyped) {
			buf.WriteString("\nimport (\n")
...
func resolveImports(srcFile []byte, imports []string) *bytes.Buffer {
...
func parsePackage(source, structName string) (code []byte, imports []string, err error) {
	...
	for i := 0; i < s.NumFields(); i++ {
		...
		tag := s.Tag(i)
		fieldType, baseName, constName, value, imp, err := parseField(field, tag)
		if err != nil {
			return nil, nil, err
		}
		bName := []rune(baseName)
...
func parseField(field *types.Var, tag string) (fieldType, baseName, constName, value, imp string, err error) {
	tags, err := structtag.Parse(tag)
	if err != nil {
		return "", "", "", "", "", fmt.Errorf("failed to parse struct tags for field %s: %w", field.Name(), err)
	}

	fieldType, imp = parseTypeName(field.Type().String())
	...
	return fieldType, baseName, string(cName), tagNameValue, imp, nil
}
